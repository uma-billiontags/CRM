...
import { Plus, Save, ChevronDown, Pencil, Loader2, AlertCircle, RefreshCw } from "lucide-react";
...
// Field names match Django Campaign model columns exactly
...
// API row includes campaign_id (auto-generated by Django model)
...
export default function Portal_Campaigns() {
  ...
  const fetchCampaigns = useCallback(async () => {
    ...
    try {
      ...
      // Support { campaigns:[...] }, { data:[...] }, or plain array
  ...
  // ── Build FormData — keys match Django Campaign model columns exactly ────────
  const buildFormData = (f: CampaignForm): FormData => {
  ...
  // ── POST to add_campaign ────────────────────────────────────────────────────
  const postCampaign = async (f: CampaignForm): Promise<boolean> => {
    setSubmitting(true);
    setSubmitError("");
    try {
      const res = await fetch(ADD_URL, { method: "POST", body: buildFormData(f) });
      if (!res.ok) {
  ...
  const handleEdit = (c: CampaignRow) => {
    ...
    setForm(rest);
    setEditingId(campaign_id);
    setSavedFlash(false);
    setSubmitError("");
    window.scrollTo({ top: 0, behavior: "smooth" });
  };

  // ── Save → refresh list → reset form ───────────────────────────────────────
  const handleSave = async () => {
    const ok = await postCampaign(form);
    if (!ok) return;
  ...
  // ── Save & Continue Editing → refresh list → keep form ─────────────────────
  ...
  const formTitle    = isEditing ? `Edit Campaign · ${editingId}` : "New Campaign";
      ...
      <div className="grid lg:grid-cols-3 gap-5">
        ...
        <div className="lg:col-span-2 card-elevated p-6">
          <div className="flex items-center justify-between mb-5">
            ...
            {isEditing && <StatusBadge status={form.status} />}
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            {isEditing && (
              <FieldInput label="Campaign ID" value={editingId ?? ""} readOnly />
            )}
              ...
              campaigns.map((c) => (
                <tr
                  ...
                  className={`border-t border-border hover:bg-muted/30 transition-colors ${
                    editingId === c.campaign_id ? "bg-primary/5" : ""
                  }`}
                >
  ...
  return (
    <label className="block">
      ...
      <input
        ...
        className="w-full h-10 px-3 rounded-md border border-input bg-surface text-sm focus:outline-none focus:ring-2 focus:ring-ring/30 focus:border-primary read-only:opacity-60 read-only:cursor-not-allowed"
      />
    </label>
  );
}
